...
namespace PDVCSharp.Application.Services
{
    ...
    public class AuthService
    {
        ...
        //    Ele só conhece a INTERFACE (IUserRepository). Isso é o princípio da "inversão de dependência".
        ...
        // 💡 DICA: async/await permite que o app continue responsivo enquanto espera o banco responder.
        //    Sem async, a tela ficaria congelada durante a consulta.
        public async Task<bool> Login(string usuario, string senha)
        {
            // Busca o usuário no banco pelo login (case-insensitive com ToLower)
            // Where() retorna um IQueryable, e FirstOrDefaultAsync() executa a consulta no banco
